...
for args in 'nodeContext.volumeStats=true' 'volumeContext.enabled=true' 'volumeContext.namespaces[0]=INVALID' 'volumeContext.health=true'; do
...
go run ./hack/volume-context-contract "${work_dir}/default.yaml" "${work_dir}/enabled.yaml" "${work_dir}/health.yaml"
