...
func TestLostResponseExpiresWithoutController(t *testing.T) {
	...
	if _, err := f.client.Bind(context.Background(), strings.Repeat("d", 32), workload(), testIntent(), time.Now().Add(250*time.Millisecond)); err != nil {
		t.Fatal(err)
	}
	h := <-f.handles
	select {
...
func TestChangedCgroupInvalidatesLease(t *testing.T) {
	f := setup(t)
	b, err := f.client.Bind(context.Background(), strings.Repeat("e", 32), workload(), testIntent(), time.Now().Add(time.Second))
	if err != nil {
		t.Fatal(err)
	}
	h := <-f.handles
...
func TestServiceShutdownClosesRetainedHandles(t *testing.T) {
	f := setup(t)
	if _, err := f.client.Bind(context.Background(), strings.Repeat("f", 32), workload(), testIntent(), time.Now().Add(time.Second)); err != nil {
		t.Fatal(err)
	}
	h := <-f.handles
	...
	if _, err := f.client.Bind(context.Background(), strings.Repeat("1", 32), workload(), testIntent(), time.Now().Add(time.Second)); !errors.Is(err, admission.ErrUnavailable) {
...
func TestReplayStorageFailsClosed(t *testing.T) {
	...
	f.service.mu.Unlock()
	if _, err := f.client.Bind(context.Background(), strings.Repeat("2", 32), workload(), testIntent(), time.Now().Add(time.Second)); !errors.Is(err, admission.ErrCapacity) {
