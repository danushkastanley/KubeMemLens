...
Assumptions requiring provider validation:
...
- Collector and extension server: validate bounded snapshots and hold current data plus recent Pod history in memory (`internal/collector/server.go`, `internal/collector/store.go`, `internal/extension`).
- Kubernetes API server: authenticates clients, applies RBAC and proxies aggregated requests with verified identity.
- Certificate bootstrap job: short-lived Helm hook that updates one Helm-created serving Secret and one `APIService`.
...
| Serving certificate bootstrap | Secret and `APIService` names, CA bundle and expiry | Name-scoped update permissions for Helm-created objects |
| Alpha `/api/v1/*` and `/metrics` | HTTP path, query and bodies | Retained in the binary for trusted pre-v1 rollback; the production chart cannot enable or expose them |
...
| Agent operational metrics | Node-local scan and mapping counts | Loopback-only listener; no chart port, scrape annotation or remote Service |
...
| TM-011 | Unauthenticated or forged snapshot ingestion | Caller reaches the write listener | High diagnostic integrity breach | Production writes only through aggregated API; exact agent ServiceAccount; delegated SAR; direct listener disabled | ISO-LIVE-001 in [tenant isolation validation](tenant-isolation-validation.md) |
...
| TM-018 | Collector compromise becomes Kubernetes compromise | Collector process is controlled | Critical cluster impact | Auth-delegator, authentication ConfigMap read and Node list only; no Pod, Secret, workload, Node proxy or mutation access; projected token; separate bootstrap account | ISO-LIVE-008 and rendered RBAC audit |
| TM-019 | Confused-deputy authorisation mismatch | Route and SAR attributes differ | High cross-tenant impact | Construct exact verb, group, resource, subresource, namespace and name from parsed route; decide before store access | ISO-LIVE-009 and route/SAR parity tests |
| TM-020 | Certificate bootstrap or rotation widens trust | Hook account or stale CA is abused | High authentication or availability impact | Name-scoped temporary RBAC; atomic Secret/APIService update; expiry alert; hook cleanup; tested rollback | ISO-LIVE-010 install, rotate, rollback and uninstall tests |
| TM-021 | Workload metrics bypass read policy | Scraper reaches legacy `/metrics` | High confidentiality impact | Authenticated cluster-scoped metrics resource; dedicated scraper Role; disable workload-labelled direct route | ISO-LIVE-011 and metrics-only RBAC tests |
| TM-022 | Agent operational metrics reveal node-local workload density | Tenant workload reaches an agent Pod | Medium confidentiality impact | Bind metrics to loopback; advertise no container port, scrape annotation or Service; require an explicit local override | Pod-proxy denial, chart render checks and agent metrics tests |
...
| Agent metrics and scanner errors | Pod reachability and raw cgroup errors must not expose node-local density or runtime identifiers | Loopback chart checks, bounded agent logging tests and ISO-LIVE-007 |
