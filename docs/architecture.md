...
6. The collector stores the latest container snapshots plus a bounded rolling Pod history in memory, deriving event and scan/steal/refault/major-fault deltas from consecutive container instances. It defaults to 5,000 node records, 100,000 current containers and 16 MiB per encoded read response. The server scans immutable node shards, retains at most 501 opaque identities while selecting a 500-item keyset page, and aggregates only the selected Pods or workloads. Nested evidence has its own byte budget, aggregate construction is serialised, and authenticated admission defaults to four requests. The TUI's initial Pod view uses a container-free Pod summary projection; full container pages load asynchronously and replace evidence atomically. History defaults to 15 minutes, 181 points per Pod instance, 1,000 total series and 20 returned instances per request.
...
The current chart does not enable this producer.
