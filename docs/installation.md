...
Current state defaults to at most 5,000 node records and 100,000 container snapshots, with an 8 MiB per-node snapshot request ceiling, 500 items per keyset page, a 16 MiB encoded JSON response ceiling and four admitted authenticated reads. The collector requests 192 MiB of memory and has a 256 MiB limit. Pod and workload pages build only selected identities and reserve half of the response ceiling for nested container evidence; aggregate construction is serialised. History defaults to 15 minutes, at most 181 points per Pod instance, 1,000 series in total and 20 returned instances for one Pod lookup. Tune `collector.ingestion.maxSnapshotBytes`, `collector.store`, `collector.read`, `collector.maxResponseBytes` and `collector.history` only from measured cluster density and within the collector Pod's memory limit. Capacity breaches fail explicitly; they do not silently evict current nodes or truncate API results.

The [local 5,000-container, 30-minute kind profile](qualification-results/rc-5000-local-kind-2026-08-26.md) measured agent and collector p95 memory above the former scheduling requests while remaining below their limits. The resulting recommendation raises requests to 96 MiB per agent and 192 MiB for the collector, retaining the measured 128 MiB and 256 MiB limits. The 16 GiB Docker allocation in the result is a local harness requirement, not a production-node recommendation. Store ceilings remain rejection bounds rather than live sizing claims. Re-run the [scale qualification gate](scale-qualification.md) when the agent interval, limits, live density, cluster shape or provider changes.
