...
| Live scale | Only the live container, node and refresh profile measured and published for the release candidate. Configured store ceilings are rejection bounds, not scale claims. | Development smoke and synthetic capacity tests. The declared `rc-5000` profile has no accepted result yet. | PROD-007 and the [scale gate](scale-qualification.md) | Qualification required |
